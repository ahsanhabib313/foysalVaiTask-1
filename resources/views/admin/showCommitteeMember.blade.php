@extends('admin.layout.adminPanel')
@section('title', 'Show Committee Member')
@section('content')
  <div id="show-advertising-logo" class="middle-content">
      <div class="container">
        <div class="row text-dark">
            <div class="col-lg-12">
                <h3 class="text-center my-3 text-white" >All Committee Members</h3>
            </div>
            <div class="col-lg-6 offset-lg-3">
                @if (Session::has('success'))
                    <div class="alert alert-success">
                        {{Session::get('success')}}
                    </div>
                @endif
            </div>
            <div class="col-lg-12">
                <table class="table table-responsive-xl  table-striped text-dark text-center">
                    <thead class="thead-dark">
                        <tr>
                            <th>SL.</th>
                            <th> Name</th>
                            <th> Designation</th>
                            <th> Commmittee</th>
                            <th> Photo</th>
                            @if (Auth::user()->role == 1)
                            <th>Action </th>
                            @endif
                        </tr>
                    </thead>
                    <tbody>
                        @isset($committeeMembers)
                                @foreach ($committeeMembers as $committeeMember)

                                    <tr class="bg-light">
                                        <td>{{$loop->index + 1}}</td>
                                        <td>
                                            {{ $committeeMember->name }}
                                        </td>
                                        <td>
                                            {{ $committeeMember->designation }}
                                        </td>
                                        <td>
                                            {{ $committeeMember->committee->name }}
                                        </td>
                                        <td>
                                            <img src="{{ asset('img/committeeMember/'.$committeeMember->photo) }}" height="100" width="100" alt="committee member photo">
                                        </td>
                                        
                                        @if (Auth::user()->role == 1)
                                        <td>
                                            <a class="btn btn-success" href="{{route('committeeMember.edit', $committeeMember->id)}}">Edit</a>
                                            <form style="display: inline-block" action="{{route('committeeMember.destroy', $committeeMember->id)}}" method="POST">
                                            @csrf
                                            @method('DELETE')
                                            <button class="btn btn-danger" type="submit">Delete</button>
                                            </form>
                                        </td>
                                        @endif
                                    </tr>

                                @endforeach
                            
                        @endisset
                    </tbody>
                </table>
            </div>
        </div>
    </div>
  </div>
    
@endsection